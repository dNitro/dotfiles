...
function! vison#complete(findstart, base)
   if a:findstart
    ...
    let b:compl_context = [l:line_str[0:compl_begin]]
    echom 'context: ' . string(b:compl_context)

    " Get query from the current buffer.
    let [b:type, b:query] = vison#resolver#get_query(getline(0, line('.') -1 ) + b:compl_context)

    " Search backwards for start of identifier (iskeyword pattern)
    let l:start = col('.') - 1
    while l:start > 0 && l:line_str[l:start - 1] =~ '\k'
      let l:start -= 1
    endwhile
    return l:start
  else
    echom 'base: ' . a:base
    " let context = matchstr(b:compl_context, '^\s*"\zs.*')
