...
" Pattern to check if scss is the compiler
let s:pat = '^' . current_compiler

" Get a `makeprg` for the current filename.
function! s:GetMakePrg()
  return g:scss_compiler .
  \      ' ' . g:scss_make_options .
  \      ' ' . fnameescape(expand('%'))
endfunction

" Set `makeprg` and return 1 if scss is still the compiler, else return 0.
...
call s:SetMakePrg()
call scss#ScssSetUpErrorFormat()

function! s:ScssMakeDeprecated(bang, args)
  echoerr 'ScssMake is deprecated! Please use :make instead, its behavior ' .
  \       'is identical.'
  sleep 5
...
command! -bang -bar -nargs=* ScssMake
\        call s:ScssMakeDeprecated(<q-bang>, <q-args>)

" Set `makeprg` on rename since we embed the filename in the setting.
augroup ScssUpdateMakePrg
  autocmd!

  " Update `makeprg` if scss is still the compiler, else stop running this
  " function.
  function! s:UpdateMakePrg()
    if !s:SetMakePrg()
      autocmd! ScssUpdateMakePrg
    endif
  endfunction
